...
def get_embeddings(model, paths, batch_size, image_size, gpu_fraction):
    ...
    with tf.Graph().as_default():
        with tf.compat.v1.Session(
                config=tf.compat.v1.ConfigProto(gpu_options=gpu_options, log_device_placement=False)) as sess:
            # Load the model
    ...
    if input_type == "w":
        ...
        if save_video:
            ...
            output_video = cv2.VideoWriter(
                "/media/linh/hdd4TA/Automated-Attendance-System-using-CNN-master/output_recognition/Output_" + data_type,
                video_format, fps, frame_size)
            ...
            if save_video or display_output or save_images:
                for i in range(bb.shape[0]):
                    ...
                    if best_class_probabilities[i] > classifier_threshold:
                        cv2.putText(image, class_names[best_class_indices[i]], (int(bb[i][0] + 1), int(bb[i][1]) + 10),
